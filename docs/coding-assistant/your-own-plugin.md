...
title: Implement your own Codiga plugin
description: Plugin specification for the Codiga coding assistant
keywords:
  [
    coding assistant,
...
In this documentation you'll find everything you need to know to create your own plugin for Codiga's coding assistant in any IDE. Taking as a reference the [VSCode plugin specification](https://doc.codiga.io/docs/coding-assistant/coding-assistant-vscode-spec/)
...
A smart code snippet is a piece of code that you can share and reuse. In Codiga you can add some metadata to a code snippet:
  ...
  - **Imports (Optional)**: The piece of code that has the imports required for the recipe to work. For example: `import React from 'react'` would be a value import value.
...
You can see how to generate an API token in our [VS Code Integration documentation](http://localhost:3000/docs/coding-assistant/coding-assistant-vscode/#linking-your-codiga-account). 
...
The current standard way to create a recipe from a plugin is by redirecting the user to the [snippet creation page](https://app.codiga.io/assistant/recipe/create) with some extra parameters taken from the IDE. You can read a little bit more about this in our blog post: [How to share code snippets in 2022](https://www.codiga.io/blog/share-code-snippets-2022/).
...
For example if I want to create a [snippet in Javascript to create a React component with props](https://app.codiga.io/hub/recipe/266/view). I can redirect the user to the following url, which has all parameters described above:
...
There are relevant variables that you can add to a code snippet that will help injecting relevant contextual information in the plugins. For more information refer to the [recipe variables documentation](https://doc.codiga.io/docs/coding-assistant/coding-assistant-recipe-variables/).

# Snippet/Recipe usage

Besides being able to create snippets and recipes, you are also able to search them and use them from the IDEs with different methods such as: `shortcuts`, `semantic search`, and `comments`.

## Fingerprint

A fingerprint in Codiga is a random string or signature that represents a session in any editor, it is usually stored in the local storage of the snippet or the browser and it's used for tracking purposes. Many of the graphql queries require it as a parameter. 
...
Shortcuts, as explained above, are an expression that the user can type in the editor and will pop up a set of suggestions in the IDE. For a reference you can see how they work in VS Code in our official [Shortcuts are now Available on VS Code](https://www.codiga.io/blog/shortcuts-in-vscode/) blog post.
...
There are some extra parameters apart from the described above which might be useful in this query:
    - `fingerprint`: Random string useful to track a session.
    - `onlyPrivate`: Return only private recipes. API Token header has to be set in order to retrieve private user recipes.
    - `onlyPublic`: Return only public recipes.

There is another useful query for shortcut management:
...
This query retrieves the last creation timestamp for a specific set of `dependencies` or `language`. 
...
The relevant query for both `Comments` and `Quick pick` search is:
